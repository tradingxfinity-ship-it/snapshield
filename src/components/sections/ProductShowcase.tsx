...
export default function ProductShowcase() {
  const swatches = products.map((p) => p.accent).slice(0, 16);
        ...
        <Reveal delay={0.1}>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-2">
            {swatches.map((c, i) => (
              <span
                key={i}
                className="h-6 w-6 rounded-full shadow-soft ring-2 ring-white"
                style={{ background: c }}
              />
            ))}
            <span className="rounded-full bg-navy px-3 py-1 text-xs font-bold text-white">+ many more</span>
          </div>
